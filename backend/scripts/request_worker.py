...
# --- دالة إضافة روابط المشاهدة ---
def generate_watch_urls(tmdb_id, kind):
    # إذا كان النوع movie نستخدم مسار الأفلام، وإلا نستخدم tv للمسلسلات/الأنمي/الكوري
...
def clean(x, kind):
    ...
    return {
        ...
        'genres': [],
        # --- تم إضافة الروابط هنا ---
        'watch_urls': generate_watch_urls(tmdb_id, kind),
...
def search(title, kind):
    ...
    for x in data.get('results', []):
        ...
        if final_kind == 'anime' and not (lang == 'ja' or country == 'JP'): continue
        if final_kind == 'kdrama' and not (lang == 'ko' or country == 'KR'): continue
...
def main():
    ...
    requests_log = load(DATA / 'requests.json')
    requests_log.insert(0, {'title': args.title, 'kind': kind, 'status': 'found' if results else 'not_found', 'count': len(results), 'created_at': time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime())})
    write(DATA / 'requests.json', requests_log[:200])
